...
contract DBXen is ERC2771Context, ReentrancyGuard, IBurnRedeemable {
     ...
     */
    DBXenERC20 public dxn = DBXenERC20(0x80f0C1c49891dcFDD40b6e0F960F84E6042bcB6F);

    /**
     * XEN Token contract.
     * Initialized in constructor.
     */
    MockToken public xen;
     ...
     */
    event FeesClaimed(uint256 indexed cycle, address indexed account, uint256 fees);

    /**
     * @dev Emitted when `account` stakes `amount` DBX tokens through
     * {stake} in `cycle`.
     */
    event Staked(uint256 indexed cycle, address indexed account, uint256 amount);

    /**
     * @dev Emitted when `account` unstakes `amount` DBX tokens through
     * {unstake} in `cycle`.
     */
    event Unstaked(uint256 indexed cycle, address indexed account, uint256 amount);
     ...
     */
    function burnBatch(uint256 batchNumber) external payable nonReentrant gasWrapper(batchNumber) {
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        if ((cycleToSet != accFirstStake[_msgSender()] && cycleToSet != accSecondStake[_msgSender()])) {
    ...
    function unstake(uint256 amount) external nonReentrant {
        ...
        require(amount <= accWithdrawableStake[_msgSender()], "DBXen: amount greater than withdrawable stake");
    ...
    function updateStats(address account) internal {
        if (currentCycle > lastActiveCycle[account] && accCycleBatchesBurned[account] != 0) {
        ...
        if (accFirstStake[account] != 0 && currentCycle > accFirstStake[account]) {
